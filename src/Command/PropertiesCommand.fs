...
module PropertiesCommand =
    ...
    let private checkWithStorage output notify currentValues storage  =
        let allPreviousValues = storage.Load() |> List.map (fun (p: GuildWars.Property) -> if p.Status = "Odstraněná" then Removed p else Old p)
        ...
        let currentValues =
            ...
            |> List.map (fun p ->
                ...
                let previousStatus =
                    ...
                    | Some previous ->
                        let p: GuildWars.Property = previous |> SearchResult.value
                        p.Status
                    | _ -> ""
        ...
        let outputValues title values =
            values
            |> List.map (SearchResult.value >> (fun (p: GuildWars.Property) -> [ p.Id; p.Status ]))
        ...
        let notification title values =
            [
                ...
                | values ->
                    ...
                    yield!
                        values
                        |> List.groupBy (SearchResult.value >> GuildWars.Property.searchTitle)
    ...
    let execute: ExecuteCommand = fun (input, output) ->
        ...
        let fileStorage =
            let create path =
                FileStorage.create (path |> Path.GetFullPath)
                    (GuildWars.Property.id >> Key)
                    GuildWars.Property.serializeToJson
                    GuildWars.Property.parseJson
        ...
        let googleSheets =
            ...
            | Some { GoogleSheets = Some config } ->
                GoogleSheets.create config
                    (GuildWars.Property.id >> Key)
                    (GuildWars.Property.serializeToSeparatedValues ";")
                    (GuildWars.Property.parseValues ";")
                |> Some
            | _ -> None
        ...
        let results =
            config
            |> Option.map (fun { GuildWars = guildWars } -> guildWars)
            |> Option.defaultValue []
            |> List.map GuildWars.fetchProperties
            |> Async.Parallel
        ...
        | Ok currentValues ->
            ...
            if output.IsVerbose() then
                ...
                // todo if verbose
                currentValues
                |> List.groupBy GuildWars.Property.searchTitle
