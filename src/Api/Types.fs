...
module CurrencyName =
    ...
    let value = function
        ...
        | Laurel -> "laurel"
        | Gem -> "gem"
        | AscalonianTear -> "ascalonian tear"
        | FractalRelic -> "fractal relic"
        | ManifestoOfTheMoletariat -> "manifesto of the moletariat"
        | DeadlyBloom -> "deadly bloom"
        | SymbolOfKoda -> "symbol of koda"
        | FlameLegionCharrCarvings -> "flame legion charr carvings"
        | BadgeOfHonor -> "badge of honor"
        | GuildComendation -> "guild comendation"
        | TransmutationCharge -> "transmutation charge"
        | AirshipPart -> "airship part"
        | LeyLineCrystal -> "ley line crystal"
        | LumpOfAurilium -> "lump of aurilium"
        | SpiritShard -> "spirit shard"
        | PristineFractalRelic -> "pristine fractal relic"
        | Geode -> "geode"
        | WvWSkirmishClaimTicket -> "wv w skirmish claim ticket"
        | BanditCrest -> "bandit crest"
        | MagnetiteShard -> "magnetite shard"
        | ProvisionerToken -> "provisioner token"
        | PvPLeagueTicket -> "pv p league ticket"
        | UnboundMagic -> "unbound magic"
        | AscendedShardOfGlory -> "ascended shard of glory"
        | TradeContract -> "trade contract"
        | ElegyMosaic -> "elegy mosaic"
        | TestimonyOfHeroic -> "testimony of heroic"
        | ExaltedKey -> "exalted key"
        | Machette -> "machette"
        | PactCrowbar -> "pact crowbar"
        | VialOfChak -> "vial of chak"
        | ZephyriteLockpick -> "zephyrite lockpick"
        | TradersKey -> "traders key"
        | VolatileMagic -> "volatile magic"
        | RacingMedailon -> "racing medailon"
        | MistbornKey -> "mistborn key"
        | FestivalToken -> "festival token"
        | CacheKey -> "cache key"
        | RedProphetShard -> "red prophet shard"
        | GreenProphetShard -> "green prophet shard"
        | BlueProphetCrystal -> "blue prophet crystal"
        | BlueProphetShard -> "blue prophet shard"
        | WarSupplies -> "war supplies"
        | TyrianDefenseSeal -> "tyrian defense seal"
        | id -> sprintf "Not defined (%A)" id
